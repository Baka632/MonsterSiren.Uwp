...
namespace MonsterSiren.Uwp.Helpers.ConnectedAnimations;
            ...
            if (props.IsListAnimation && parameter != null)
            {
                foreach (var listAnimProperty in props.ListAnimProperties)
                {
                    if ((listAnimProperty.ListViewBase.ItemsSource is IEnumerable<object> items &&
                        ...
                        listAnimProperty.ListViewBase.ItemsSource is System.Collections.IEnumerable valItems &&
                        valItems.Cast<object>().Contains(parameter)))
                    {
                        try
                        {
